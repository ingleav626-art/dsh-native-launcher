...
-  **增强设置页（完整表单）**：设置侧边栏 "WebUI 启动器" 卡片即**可视化配置表单**——启动命令 / 端口 / 快捷方式名 / 自动打开 / 打开方式 / 托盘 / 托盘通知 / 关闭语义（防抖秒数·确认窗口秒数）/ **模块开关组**，全部可调、保存即持久化（官方 settings 文档）；生成物类改动保存后自动重建并热重启托盘，即时生效 + 独立"通知"设置 section（完成原因开关、关键词规则、等待确认开关、测试按钮）
-  **一键卸载**：设置页内自绘确认框（可选：同时清除全部个性化配置）——停止托盘与 dsh 服务（约 6 秒自动停止，无需 taskkill）、删桌面快捷方式、清生成物与通知注册表项、从 profile 自移除条目（自动备份）；全程写入独立审计日志 `uninstall.log`（该日志不在任何清理清单中，失败可溯源）
-  **模块化整合包**：本体之外的功能全部模块化（`lib/modules/<id>/` 目录约定），设置页独立开关、单模块故障自动隔离；自带任务通知模块为首个样板
-  **零依赖**：仅 node builtins + Windows 自带工具

###  下一步实现（规划中）

- **插件生态**：external/dev 模块动态发现（npm 安装第三方模块到数据目录）、vendored 模块的上游更新检测
