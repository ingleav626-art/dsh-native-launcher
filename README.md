...
>
> **版本要求**：需要 **dsh >= 0.1.5-rc.2**（已适配 dsh 0.1.7 并以其为真机验证基准；`--no-open` 参数自 rc.8 起、任务通知所依赖的官方能力自 0.1.5 起）。本插件以该区间为基线持续适配，**不再针对更低版本做适配与验证**——使用更低版本 dsh 请安装对应的历史插件版本（见下方「版本状态」）。dsh 版本过低时，插件启动会在日志中给出升级提示。
>
...
- **任务通知**：任务完成、出错、中止、被阻塞、达到上限，以及等待审批 / 回答 / 计划审阅时，通过 Windows 原生通知提醒并进入通知中心；关闭全部页面后由后台补发，确保消息不遗漏。设置页有「**发送测试通知**」按钮，一键验证「模块 → 通知通道 → 托盘 → 系统」整条链路（排错第一站）。提示音三档可选：系统默认、静音，或**自定义音效**——在设置页选择本地音频文件（wav / mp3 / wma），改动即时生效，配完紧接着点「发送测试通知」就能试听。
...
自定义通知音效已随 v1.0.0 交付（设置页选择本地音频文件，wav / mp3 / wma）——这是计划中的最后一项新增功能。此后以维护为主，用户反馈的问题会按实际需要评估处理。
...
> 当前版本 **v1.0.0**。各版本详情见 GitHub Releases；里程碑之外的迭代见提交记录。

- **v1.0.0 — 首个正式版**。新增**自定义通知音效**：设置页选择本地音频文件（wav / mp3 / wma），弹出通知即播放，改动即时生效；适配 dsh 0.1.7 的设置机制（0.1.5-rc.2 以上均兼容）；修复通知设置偶发失效与自动打开探测超时的两个真机问题。**升级注意**：从旧版本（插件 v0.4.2 + dsh 0.1.6 及以下）升级后，通知开关与关键词规则需**重新配置一次**（设置存储位置随 dsh 新机制调整，一次性成本）；重启 dsh 后托盘自动换新，无需手动处理
