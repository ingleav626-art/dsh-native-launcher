...
>
> **版本要求**：需要 **dsh >= 0.1.0-rc.8**（rc.8 起官方默认自动打开浏览器，插件通过 `--no-open` 避免双开；该参数 rc.8 起支持）。**任务通知功能需要 dsh >= 0.1.5-rc.2**。若 dsh 版本过低，插件启动时会记录日志提醒升级：`npm install -g @deepseek-ai/dsh@0.1.0-rc.8`
>
...
> 当前版本 **v0.4.0**。各版本详情见 GitHub Releases；里程碑之外的迭代见提交记录。

- **v0.4.0** — 启动链路稳定性修复（页面已开时双击快捷方式正确唤起窗口 / 日志自动打开，均适配 dsh 登录鉴权）；**两个新功能**：任务通知卡片点击后直接回到 DeepSeek（唤起已有窗口，不重复开页）；开机自动启动 DSH WebUI（默认关，设置页开启）；工程全面 TypeScript 化并加入生成脚本自动守卫
