...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  exit 0
    ...
    `$webuiUrlFile = '${join2(launcherDir, "webui-url.txt").replace(/'/g, "''")}'`,
    ...
    "        if ($appId) {",
    "          $toastNode.SetAttribute('launch', 'shell:AppsFolder\\' + $appId + '!App')",
    "        } else {",
    ...
    "            }",
    "          }",
    "          $toastNode.SetAttribute('launch', $u)",
    "        }",
    "      } catch { }",
    "      if ($persistent) {",
