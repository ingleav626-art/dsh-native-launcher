...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const openScript = openScriptPath || join2(launcherDir, "open-webui.ps1");
  const toastVbs = [
  ...
  writeFileSync(join2(launcherDir, "toast-open.vbs"), toastVbs, "utf-8");
  const toastVbsInline = join2(launcherDir, "toast-open.vbs").replace(/'/g, "''");
  const ps = [
  ...
  exit 0
    ...
    `# 点击 Toast 回到 DeepSeek：注册 dsh-webui: 协议（幂等）——Toast activationType=protocol`,
    `# 的 launch 属性触发本协议 → open-webui.ps1（PWA 窗口优先 / 聚焦已有窗口）。`,
    `# 用注册表 provider 而非 reg.exe：协议 command 值内嵌双引号（包路径），`,
    `# reg.exe 在 cmd/PS 引号转义链里必截断（实测 default 值为空）——原生写值零转义问题。`,
    `# 关键结构：协议键下必须有名为 "URL Protocol" 的空值，缺了它 ShellExecute 不认作`,
    `# 可执行协议（真机实锤：点击 Toast 提示"没有找到可跳转的链接"）。`,
    ...
    `  [SHN]::SHChangeNotify(0x08000000, 0, [IntPtr]::Zero, [IntPtr]::Zero)`,
    `} catch { }`,
    ...
    "      # 点击卡片回到 DeepSeek：protocol activation（无需 COM 回调常驻）——dsh-webui: 协议",
    "      # 由托盘启动时注册，指向 open-webui.ps1（PWA 窗口优先 / 聚焦已有窗口）",
    "      try {",
    "        $toastNode = $template.DocumentElement",
    "        $toastNode.SetAttribute('activationType', 'protocol')",
    "        $toastNode.SetAttribute('launch', 'dsh-webui:open')",
    "      } catch { }",
    "      if ($persistent) {",
