...
function writeOpenScript(launcherDir, port, openMode, shortcutName, pwaAppId) {
  ...
  const ps = [
    ...
    `$tokFile = '${join2(launcherDir, "webui-url.json").replace(/'/g, "''")}'`,
    ...
    `$openLogPath = '${join2(logsDirOf(launcherDir), "open-webui.log").replace(/'/g, "''")}'`,
    ...
    "",
    // ⚠️ 禁止在此用 shell:AppsFolder / --app-id "激活"PWA——对已运行实例会双开新窗口
    // （项目注释与 2026-09-14 实测两度确认：Chromium 的 AUMID 激活不带聚焦语义）。
    // 已运行实例的唯一正确处理 = Focus-AppWindow 聚焦（下方）。
    "",
  ...
  writeFileSync(join2(launcherDir, "open-webui.ps1"), "\uFEFF" + ps, "utf-8");
}
function writeLauncherFiles(launcherDir, launchCommand, port, trayPath, openScriptPath) {
  const url = `http://127.0.0.1:${String(port)}`;
  const openLine = openScriptPath ? `  powershell -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "${openScriptPath}"` : `  start "" "${url}"`;
  const cmd = [
    "@echo off",
    // 日志目录：launch.cmd 可能在任何 dsh 进程之前跑（首次双击），故自己保证目录存在——
    // 否则 `>>` 重定向到一个不存在的目录会报错并打乱下面的 if/else 分支判断。
    `set "LOGDIR=${join2(launcherDir, "logs")}"`,
    ...
    `powershell -NoProfile -NonInteractive -WindowStyle Hidden -Command "$ErrorActionPreference = 'SilentlyContinue'; $u = 'http://127.0.0.1:${String(port)}/'; $tf = '${join2(launcherDir, "webui-url.txt")}'; if (Test-Path $tf) { $t = (Get-Content $tf -Raw).Trim(); if ($t) { $p2 = [System.Uri]$t; $b2 = [System.Uri]$u; if ($p2.Host -eq $b2.Host -and $p2.Port -eq $b2.Port) { $u = $t } } }; try { $null = Invoke-WebRequest -Uri $u -UseBasicParsing -TimeoutSec 3; exit 0 } catch { if ($_.Exception.Response) { exit 0 }; exit 1 }"`,
    "if %errorlevel%==0 (",
    ...
    `  start "" /min powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "${join2(launcherDir, "launch-ready.ps1")}" ${String(port)} "${join2(logsDirOf(launcherDir), "launch.log")}"`,
    "  set DSH_LAUNCHER=1",
    ...
    ...launchCommand.split(/\s+/)[0].includes("\\") || launchCommand.split(/\s+/)[0].includes("/") ? [`  ${launchCommand} < nul`] : launchCommand.split(/\s+/)[0] === "dsh" ? [
      "  where dsh >nul 2>nul",
      "  if errorlevel 1 (",
  ...
  writeFileSync(join2(launcherDir, "launch.ps1"), "\uFEFF" + launchPs, "utf-8");
  const readyPs = [
  ...
  writeFileSync(join2(launcherDir, "launch-ready.ps1"), readyPs, "utf-8");
  writeFileSync(join2(launcherDir, "launch.cmd"), cmd, "utf-8");
  const vbs = [
    ...
    `ws.Run "powershell -NoProfile -ExecutionPolicy Bypass -File ""${join2(launcherDir, "launch.ps1")}"" -Port ${String(port)} -LauncherDir ""${launcherDir}"" -OpenScript ""${join2(launcherDir, "open-webui.ps1")}""", 0, False`
  ].join("\r\n");
  writeFileSync(join2(launcherDir, "launcher.vbs"), vbs, "utf-8");
  writeFileSync(join2(launcherDir, "launcher-visible.vbs"), vbs.replace(", 0, False", ", 1, False"), "utf-8");
}
export {
  writeLauncherFiles,
  writeOpenScript
