...
function writeLauncherFiles(launcherDir, launchCommand, port, trayPath, openScriptPath) {
  ...
  const vbs = [
    ...
    `ws.Run "powershell -NoProfile -ExecutionPolicy Bypass -File ""${join2(launcherDir, "launch.ps1")}"" -Port ${String(port)} -LauncherDir ""${launcherDir}"" -OpenScript ""${join2(launcherDir, "open-webui.ps1")}""", 1, False`
  ].join("\r\n");
