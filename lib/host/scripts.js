...
function writeLauncherFiles(launcherDir, launchCommand, port, trayPath, openScriptPath) {
  ...
  const cmd = [
    ...
    ...launchCommand.split(/\s+/)[0].includes("\\") || launchCommand.split(/\s+/)[0].includes("/") ? [`  ${launchCommand}`] : launchCommand.split(/\s+/)[0] === "dsh" ? [
      "  where dsh >nul 2>nul",
      "  if errorlevel 1 (",
  ...
  const vbs = [
    ...
    `ws.Run "cmd /c ""${join2(launcherDir, "launch.cmd")}""", 1, False`
  ].join("\r\n");
