...
import { appendFileSync, copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, unlinkSync as unlinkSync2, writeFileSync as writeFileSync2 } from "node:fs";
import { join as join2 } from "node:path";
import { spawn, spawnSync } from "node:child_process";

// src/host/io/soundFile.ts
import { unlinkSync, writeFileSync } from "node:fs";
import { join } from "node:path";
var NOTIFY_SOUND_BASENAME = "notify-sound";
var NOTIFY_SOUND_EXTENSIONS = [".wav", ".mp3", ".wma"];
var MAX_SOUND_BYTES = 2 * 1024 * 1024;
function notifySoundExtOf(name) {
  const dot = name.lastIndexOf(".");
  if (dot < 0 || dot === name.length - 1) return null;
  const ext = name.slice(dot).toLowerCase();
  return NOTIFY_SOUND_EXTENSIONS.includes(ext) ? ext : null;
}
function isStrictBase64(data) {
...
function saveNotifySound(launcherDir, name, dataBase64) {
  ...
  try {
    for (const candidate of NOTIFY_SOUND_EXTENSIONS) {
      try {
        unlinkSync(join(launcherDir, NOTIFY_SOUND_BASENAME + candidate));
      } catch {
      }
    }
  ...
  const nativeLauncherRpcHandler = async (endpoint, _payload) => {
    switch (endpoint) {
      case "config.get": {
        ...
        return {
          ...
          value: {
            ...
            shortcutExists: existsSync(join2(io.resolveDesktopPath() ?? "", `${live.shortcutName ?? deps.shortcutName}.lnk`))
          }
        };
      }
      case "config.set": {
        ...
        try {
          ...
          try {
            ...
            io.writeLauncherFiles(launcherDir, fresh.launchCommand ?? deps.launchCommand, fPort, fTrayEnabled ? join2(launcherDir, "tray.ps1") : null, join2(launcherDir, "open-webui.ps1"));
            if (fTrayEnabled) io.writeTrayScript(launcherDir, fPort, join2(launcherDir, "dsh-webui.ico"), join2(launcherDir, "open-webui.ps1"), pwaId);
            if (changedKeys.includes("shortcutName")) {
              const oldName = String(prevValues.shortcutName ?? "").trim();
              if (oldName && oldName !== fShortcutName) {
                const oldLnk = join2(io.resolveDesktopPath() ?? "", `${oldName}.lnk`);
            ...
            if (trayChanged.length > 0) {
              ...
              if (fTrayEnabled) io.startTrayProcess(launcherDir, join2(launcherDir, "tray.ps1"), fSurvives, logMsg, logWarn, logFail);
      ...
      case "shortcut.recreate": {
        ...
        const lnk = join2(io.resolveDesktopPath() ?? "", `${liveName}.lnk`);
      ...
      case "launcher.uninstall": {
        const ULOG = join2(io.logsDirOf(launcherDir), "uninstall.log");
        ...
        try {
          ...
          if (desktop) targets.add(join2(desktop, `${liveShortcutName}.lnk`));
          let removedLnk = 0;
          const failedLnks = [];
        ...
        try {
          ...
          const artifacts = [
            ...
            "shortcut-registry.txt",
            "webui-url.json",
            join2("logs", "native-launcher.log"),
            join2("logs", "native-launcher.prev.log"),
            join2("logs", "launch.log"),
            join2("logs", "tray-exit.log"),
            join2("logs", "tray-notify.log"),
            join2("logs", "pwa-scan.log"),
            join2("logs", "open-webui.log"),
            join2("logs", "test-results.log"),
            "tray-notify.json",
            "tray-version.txt",
            "tray-pid.txt",
            "tray-state.json",
          ...
          for (const name of artifacts) {
            const p = join2(launcherDir, name);
        ...
        try {
          ...
          try {
            const pm = deps.getService("pluginManager");
          ...
          const profilesRoot = join2(home, "profiles");
          let touched = false;
          if (!officialRemoved && existsSync(profilesRoot)) {
            for (const entry of readdirSync(profilesRoot, { withFileTypes: true })) {
              if (!entry.isDirectory()) continue;
              const pkgPath = join2(profilesRoot, entry.name, "package.json");
          ...
          try {
            const profilesRoot4Link = join2(process.env.DSH_HOME ?? join2(process.env.USERPROFILE ?? "", ".dsh"), "profiles");
            let unlinked = 0;
            if (existsSync(profilesRoot4Link)) {
              for (const entry of readdirSync(profilesRoot4Link, { withFileTypes: true })) {
                if (!entry.isDirectory()) continue;
                const linkPath = join2(profilesRoot4Link, entry.name, "node_modules", "dsh-native-launcher");
        ...
        try {
          const home2 = process.env.DSH_HOME ?? join2(process.env.USERPROFILE ?? "", ".dsh");
          const profilesRoot2 = join2(home2, "profiles");
          if (existsSync(profilesRoot2)) {
            for (const entry of readdirSync(profilesRoot2, { withFileTypes: true })) {
              if (!entry.isDirectory()) continue;
              const dir = join2(profilesRoot2, entry.name);
              const pkgPath = join2(dir, "package.json");
              try {
                if (!existsSync(pkgPath)) continue;
                const rawPkg = readFileSync(pkgPath, "utf-8");
                if (!rawPkg.includes('"dsh-native-launcher"') && !existsSync(join2(dir, "node_modules", "dsh-native-launcher"))) continue;
                for (const lf of ["pnpm-lock.yaml", "package-lock.json", "yarn.lock"]) {
                  if (existsSync(join2(dir, lf))) {
