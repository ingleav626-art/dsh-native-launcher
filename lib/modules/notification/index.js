...
function resolveTraySound(settings) {
  const mode = settings.sound;
...
function createNotifier(deps, options = {}) {
  ...
  const payload = (title, body, tag) => {
    const base = persistent() ? { title, body, tag, persistent: true } : { title, body, tag };
    const traySound = sound();
    return traySound === void 0 ? base : { ...base, sound: traySound };
  };
...
// src/modules/notification/host/presence.ts
var PRESENCE_TTL_MS = 15e3;
function isPresenceReport(value) {
  if (typeof value !== "object" || value === null) return false;
  const record = value;
  if (typeof record.visible !== "boolean") return false;
  return record.activeSessionId === void 0 || typeof record.activeSessionId === "string";
}
function createPresenceTracker(now = Date.now) {
  let last;
  return {
    report(raw) {
      ...
      last = {
        state: {
          visible: raw.visible,
          ...raw.activeSessionId === void 0 ? {} : { activeSessionId: raw.activeSessionId.slice(0, 200) }
        },
        at: now()
      };
      return true;
    ...
    suppresses(sessionId, settings, at = now()) {
      ...
      if (at - last.at > PRESENCE_TTL_MS) return false;
      if (!last.state.visible) return false;
      return last.state.activeSessionId === sessionId;
    }
  };
}
  ...
  return {
    ...
    testNotify() {
      try {
        ...
        const traySound = current === void 0 ? void 0 : resolveTraySound(current);
        deps.notify.notify({
          title: "任务通知测试",
          ...
          ...traySound === void 0 ? {} : { sound: traySound }
        });
