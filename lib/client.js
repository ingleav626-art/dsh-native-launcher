...
function apply(ctx) {
  ...
  const show = (title, body, tag, requireInteraction, key) => {
    ...
    // 浏览器通知降级为兜底：3 秒后查询托盘是否已消费（tray-acked），未消费才弹浏览器。
    try {
      const conn = ctx.get("connection");
      if (conn && conn.rpc) {
        conn.rpc.call("/native-launcher", "tray-notify", { title: String(title), body: String(body), tag: String(tag), key: String(key || "") });
      }
    } catch (e) {
      // connection 不可用时静默（兜底定时器会触发浏览器通知）
    }
    const fireBrowser = () => {
      const api = notificationsApi();
      ntfLog("show", { title, body, tag, api: api === void 0 ? "undefined" : "ok", permission: api === void 0 ? "?" : api.permission });
      if (api === void 0 || api.permission !== "granted") return;
      try {
        const notification = new api(title, { body, tag, requireInteraction });
        ntfLog("constructed", { tag });
        notification.onclick = () => {
          window.focus();
        };
      } catch (error) {
        ntfLog("construct-error", { tag, error: String(error && error.message ? error.message : error) });
      }
    };
    try {
      const conn = ctx.get("connection");
      if (conn && conn.rpc) {
        setTimeout(function () {
          conn.rpc.call("/native-launcher", "tray-acked", { key: String(key || "") }).then(function (res) {
            if (res && res.ok && res.value && res.value.consumed) return; // 托盘已弹，浏览器不弹
            fireBrowser();
          }).catch(function () { fireBrowser(); });
        }, 3000);
      } else {
        fireBrowser();
      }
  ...
  ctx.effect(() => {
    ...
    const off = sessions.list.subscribe(() => {
      ...
      for (const id of state.ids) {
        ...
        if (plan === null) {
          ntfLog("suppressed", { id, turn: nextTurn, origin: summary.origin, title: summary.title });
          continue;
        }
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LauncherSection(props) {
      ...
      var uninstallResult = react.useState(null);
      var setUninstallResult = uninstallResult[1];
      var clearCfg = react.useState(false);
      var setClearCfg = clearCfg[1];

      function uninstall() {
        // 危险操作：二次确认后再执行
        var wipeText = clearCfg[0] ? '\n\n已勾选「同时清除全部配置」——保存过的个性化设置将一并清空，重装后回到默认值。' : '';
        var ok = window.confirm('确定卸载 WebUI 启动器？\n\n将停止托盘、删除桌面快捷方式、清理生成文件与通知注册表，并从 dsh profile 移除本插件条目。' + wipeText + '\n\n完成后重启 dsh 即完全生效（开发链接安装不会删除源码目录）。');
        if (!ok) return;
        setUninstallBusy(true);
        setUninstallResult(null);
        rpc.call('/native-launcher', 'launcher.uninstall', { clearSettings: clearCfg[0] }).then((result) => {
      ...
      var clearCfgRow = react.createElement(
        'label',
        { style: { marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--dsw-alias-label-secondary)', cursor: 'pointer' } },
        react.createElement('input', {
          type: 'checkbox', checked: clearCfg[0],
          onChange: (e) => setClearCfg(e.target.checked),
          style: { width: 15, height: 15, accentColor: 'var(--dsw-alias-state-error-primary)' },
        }),
        '同时清除本插件保存的全部个性化配置（重装后回到默认值）',
      );
      ...
      return react.createElement('section', { style: { maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 4 } }, title, intro, content, button, clearCfgRow, uninstallButton, uninstallEl, noticeEl);
    }
