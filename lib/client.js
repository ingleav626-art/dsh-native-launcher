...
function apply(ctx) {
  ...
  const show = (title, body, tag, requireInteraction, key) => {
    ...
    try {
      const conn = ctx.get("connection");
      if (conn && conn.rpc) {
        ntfLog("tray-send", { title, tag, key: String(key || "") });
        conn.rpc.call("/native-launcher", "tray-notify", { title: String(title), body: String(body), tag: String(tag), key: String(key || "") })
          .then(function (res) {
            ntfLog("tray-send-result", { key: String(key || ""), ok: !!(res && res.ok) });
          })
          .catch(function (err) {
            ntfLog("tray-send-error", { key: String(key || ""), error: String(err && err.message ? err.message : err) });
          });
      } else {
        ntfLog("tray-send-skipped", { key: String(key || ""), reason: "no connection" });
      }
    } catch (e) {
      ntfLog("tray-send-exception", { key: String(key || ""), error: String(e && e.message ? e.message : e) });
    }
    const fireBrowser = () => {
      ...
      if (conn && conn.rpc) {
        setTimeout(function () {
          conn.rpc.call("/native-launcher", "tray-acked", { key: String(key || "") }).then(function (res) {
            var consumed = !!(res && res.ok && res.value && res.value.consumed);
            ntfLog("tray-ack", { key: String(key || ""), consumed });
            if (consumed) return; // 托盘已弹，浏览器不弹
            fireBrowser();
  ...
  ctx.effect(() => {
    ...
    const off = sessions.list.subscribe(() => {
      ...
      // 链路打点：runner 触发即留痕（会话数 + 通知模块开关状态）
      ...
      for (const id of state.ids) {
        ...
        // 链路打点：投影存在但回合未推进时留痕（本次事故的盲区——投影缺失会静默跳过）
        if (!fresh && projection !== void 0) ntfLog("advance-stale", { id, turn: nextTurn });
        if (!fresh) continue;
