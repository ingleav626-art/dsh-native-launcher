...
var COPY = {
  ...
  "settings.sound.title": "提示音",
  "settings.sound.subtitle": "任务通知弹出时的提示音。选择本地音频文件（wav / mp3 / wma，最大 2 MB）；改动即时生效，可用「测试通知」试听。",
  "settings.sound.default": "系统默认",
  "settings.sound.none": "静音",
  "settings.sound.custom": "自定义音效",
  "settings.sound.pick": "选择音效文件…",
  "settings.sound.picking": "上传中…",
  "settings.sound.current": "当前音效文件：",
  "settings.sound.nonePicked": "未选择（选好文件后自动保存）",
...
// src/modules/notification/client/soundFile.ts
var MAX_SOUND_BYTES = 2 * 1024 * 1024;
var SOUND_EXTENSIONS = [".wav", ".mp3", ".wma"];
function isAllowedSoundName(name) {
  const dot = name.lastIndexOf(".");
  if (dot < 0 || dot === name.length - 1) return false;
  return SOUND_EXTENSIONS.includes(name.slice(dot).toLowerCase());
}
async function fileToSoundUpload(file) {
...
function NotificationCard(props) {
  ...
  const [soundState, setSoundState] = (0, import_react.useState)("idle");
  const soundInputRef = (0, import_react.useRef)(null);
  const onSoundPicked = (file) => {
    if (!file) return;
    setSoundState("uploading");
    setNotice(null);
    fileToSoundUpload(file).then((payload) => {
      if (!payload.ok) {
        setNotice(payload.error);
        setSoundState("idle");
        return;
      }
      return face.sound.upload({ name: payload.name, dataBase64: payload.dataBase64 }).then((result) => {
        if (result.ok) {
      ...
      face.logger.warn(`[card] 音效上传失败：${String(error2)}`);
      setNotice(`音效上传失败：${String(error2)}`);
    }).finally(() => {
      setSoundState("idle");
    });
  };
    ...
    cardEl(
      ...
      (0, import_react.createElement)(
        "select",
        {
          ...
          onChange: (event) => {
            apply2({ sound: event.target.value });
          }
        },
        SOUND_OPTIONS.map((option) => (0, import_react.createElement)("option", { key: option.value, value: option.value }, option.label))
      ),
      settings.sound === "custom" ? (0, import_react.createElement)(
        "div",
        { className: "dsh_notification_rulesFooter" },
        ...
        (0, import_react.createElement)(
          ...
          settings.soundName === "" ? COPY["settings.sound.nonePicked"] : `${COPY["settings.sound.current"]}${settings.soundName}`
        )
      ) : null
    ),
    cardEl(
...
function parseSoundUploadResponse(result) {
  ...
  if (record.ok !== true || typeof value?.path !== "string") {
    ...
    const reason = typeof errObj?.message === "string" && errObj.message !== "" ? errObj.message : "host 拒绝了这次上传";
...
function LauncherSection(props) {
  ...
  if (meta[0].loading) {
    ...
    content = (0, import_react2.createElement)(
      ...
      rowEl("开机自启动", "开机自动启动 DSH WebUI（等同开机自动双击桌面快捷方式；改动即时生效）", toggleEl("autoStartBoot")),
      groupEl("浏览器窗口"),
