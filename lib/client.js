...
function NotificationSettingsSection({ useSettings, set, requestPermission, sendTest, t }) {
  ...
  // 权限 state 跟随浏览器真实权限：授权弹窗关闭、窗口重新聚焦时刷新，
  // 否则用户通过浏览器弹窗授权后按钮仍保持挂载时的 default/denied 禁用状态
  (0, import_react.useEffect)(() => {
    const refresh = () => setPermission(notificationsApi()?.permission ?? "denied");
...
function apply(ctx) {
  ...
  // 通知诊断上报：写 host 日志（native-launcher.log [ntf] 行），不污染浏览器 console
  ...
  const sendTest = () => {
    show(t("notify.testTitle"), t("notify.testBody"), "dsh-notification-test", false, "test");
  };
  ctx.effect(() => {
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LauncherSection(props) {
      ...
      return react.createElement('section', { style: { maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 4 } }, title, intro, body, button, noticeEl);
    }
