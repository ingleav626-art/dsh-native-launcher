// better-dsh-webui-launcher — Client half
  ...
  factory: (require) => {
    ...
    function LauncherSection(props) {
      ...
      react.useEffect(() => {
        var alive = true;
        rpc
          .call('/better-launcher', 'config.get', {})
          .then((result) => {
            if (!alive) return;
      ...
      function recreate() {
        ...
        setNotice(null);
        rpc
          .call('/better-launcher', 'shortcut.recreate', {})
          .then((result) => {
            setBusy(false);
    ...
    function apply(ctx) {
      ...
      try {
        ...
        console.error('[better-launcher] settings section registration failed:', error);
      }

      // 注入自定义 favicon + 真实 URL 的 PWA manifest：
      //   favicon → --app 独立窗口的任务栏图标
      //   manifest → host 提供的 /better-launcher/manifest.webmanifest（真实 http URL，
      //              Chromium 可安装性检查只认这个；blob/data URL 均被拒）
      try {
        ctx.connection.rpc
          .call('/better-launcher', 'icon.get', {})
          .then(function (result) {
            ...
            manifestLink.href = '/better-launcher/manifest.webmanifest';
            head.appendChild(manifestLink);
          })
          .catch(function (error) {
            console.error('[better-launcher] favicon inject failed:', error);
          });
      } catch (error) {
        console.error('[better-launcher] icon inject failed:', error);
      }
      ...
      try {
        ...
        function showCard() {
          ...
          img.src = '/better-launcher/icon.png';
          img.alt = '';
        ...
        console.error('[better-launcher] install prompt setup failed:', error);
      }
    }
