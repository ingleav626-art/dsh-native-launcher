...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LauncherSection(props) {
      ...
      // 任务通知：请求浏览器通知权限（页面内通道；页面关闭时由托盘气泡兜底）
      function enableNotify() {
        ...
        Notification.requestPermission().then(function (perm) {
          if (perm === 'granted') {
            setNotice({ kind: 'ok', text: '通知已启用：任务完成时页面内提醒；页面关闭时由托盘气泡提醒' });
            try {
      ...
      var notifyBlock = react.createElement(
        ...
        react.createElement(Row, { label: '任务通知' }, '任务完成时提醒（页面内通知 + 托盘气泡）'),
        react.createElement(
          'button',
          {
            type: 'button',
    ...
    function apply(ctx) {
      ...
      // 任务通知轮询：host 在 agent/status running→idle 时写入 notify.json，
      // 这里每 3 秒取一次，页面开着时用 Notification API 提醒（页面关了由托盘气泡兜底）。
      try {
        var notifyCursor = 0;
            ...
            .then(function (result) {
              ...
              if (at > notifyCursor) {
                ...
                if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
                  var isError = result.value.kind === 'error';
                  new Notification(isError ? 'DSH 任务出错' : 'DSH 任务完成', {
                    body: '会话 ' + (result.value.sessionId || '?') + (isError ? ' 执行出错' : ' 已完成'),
