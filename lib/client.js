...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LauncherSection(props) {
      ...
      function uninstall() {
        // 危险操作：二次确认后再执行
        var ok = window.confirm('确定卸载 WebUI 启动器？\n\n将停止托盘、删除桌面快捷方式、清理生成文件与通知注册表。\n完成后需要手动从 dsh profile 移除本插件。');
        if (!ok) return;
        setUninstallBusy(true);
        setUninstallResult(null);
        rpc
          .call('/native-launcher', 'launcher.uninstall', {})
          .then((result) => {
      ...
      if (uninstallResult[0]) {
        ...
        uninstallEl = react.createElement(
          ...
          react.createElement('div', { style: { fontWeight: 500, color: 'var(--dsw-alias-state-warning-primary)' } }, '需要你手动完成：'),
          react.createElement('ul', { style: { margin: '4px 0 0', paddingLeft: 20 } }, manualEls),
        );
      }
      ...
      return react.createElement('section', { style: { maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 4 } }, title, intro, body, button, uninstallButton, uninstallEl, noticeEl);
    }
