        ...
        switch (endpoint) {
          ...
          case 'launcher.uninstall': {
            ...
            try {
              ...
              logU('INFO', `STEP 1/5 tray-kill: taskkill exit=${ps.status}, pids=[${killed.join(',')}]`);
            } catch (error) {
            ...
            try {
              ...
              addStep(removedLnk > 0 ? `已删除 ${removedLnk} 个桌面快捷方式` : '桌面无本插件快捷方式（跳过）');
              logU('INFO', `STEP 2/5 shortcut-sweep: removed=${removedLnk}`);
            } catch (error) {
              addFail(`删除快捷方式失败（继续）: ${error}`);
            }
            try {
              ...
              addStep(`已清理生成物 ${removed} 个文件`);
              logU('INFO', `STEP 3/5 artifacts: removed=${removed}`);
            } catch (error) {
              addFail(`清理生成物失败（继续）: ${error}`);
            }
            try {
              ...
              addStep(reg.status === 0 ? '已删除通知标识注册表项 (AUMID DshNativeLauncher)' : '通知注册表项不存在（跳过）');
              logU('INFO', `STEP 4/5 registry: exit=${reg.status}`);
            } catch (error) {
              addFail(`清理注册表失败（继续）: ${error}`);
            }
            try {
