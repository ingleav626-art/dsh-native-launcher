...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const ps = [
    ...
    `if (-not $mutexAcquired) { try { Add-Content -Path '${join(launcherDir, 'tray-exit.log').replace(/'/g, "''")}' -Value ('[' + (Get-Date -Format 'HH:mm:ss.fff') + '] mutex not acquired (Local\\DshNativeLauncherTray held), exiting') -Encoding UTF8 } catch { }; exit 0 }`,
...
function killExistingTrays(launcherDir, logMsg) {
  ...
  logMsg(`[native-launcher] tray kill result: ${out || '(none)'} exit=${result.status}`);
  return out;
}
...
function startTrayProcess(launcherDir, trayPath, preferPersistent, logMsg) {
  ...
  logMsg(`[native-launcher] startTrayProcess: mode=${modeLabel}, trayPath=${trayPath}`);
  const spawnTray = (attempt) => {
    if (attempt > 3) {
      logMsg('[native-launcher] tray spawn FAILED after 3 attempts (mutex held or script error; see tray-exit.log)');
      return;
    }
    ...
    if (useWscript) {
      ...
      child.on('exit', (code, signal) => {
        logMsg(`[native-launcher] tray attempt ${attempt} (wscript run, ${modeLabel}) launcher exited code=${code} signal=${String(signal)}`);
      });
    } else {
      const child = spawn(
        'powershell',
      ...
      child.unref();
      child.on('exit', (code, signal) => {
        logMsg(`[native-launcher] tray attempt ${attempt} (direct spawn, ${modeLabel}) exited code=${code} signal=${String(signal)} (stdio=ignore; see tray-exit.log for script-level reason)`);
      });
    }
    setTimeout(() => {
      probeAlive((alive) => {
        logMsg(`[native-launcher] tray spawn attempt ${attempt}: alive=${alive}${alive ? '' : ' -> retrying'}`);
        if (!alive) spawnTray(attempt + 1);
      });
    }, 1500);
  };
  spawnTray(1);
...
function applyInner(ctx, config = {}) {
  ...
  if (dshVersion && !dshVersionGte(dshVersion, '0.1.0-rc.8')) {
    logMsg(`[native-launcher] WARNING: dsh ${dshVersion} is below 0.1.0-rc.8 — the launch command uses --no-open (rc.8+ only). Please upgrade: npm install -g @deepseek-ai/dsh@0.1.0-rc.8`);
  }
  logMsg(`[native-launcher] dsh version: ${dshVersion || '(unknown)'}`);
  ...
  try {
    if (ctx.settings) {
      ...
      settingsScope.watch(() => logMsg('[native-launcher] settings updated — restart dsh (double-click shortcut) to fully apply'));
    } else {
      logMsg('[native-launcher] settings service unavailable, using patch config only');
    }
  } catch (error) {
    // 重复注册（fiber 重载竞态）等场景：保留 patch 配置继续跑，不拖垮本体
    logMsg(`[native-launcher] settings register skipped: ${error?.message ?? error}`);
  }
      ...
      async (endpoint, _payload) => {
        switch (endpoint) {
          case 'config.get':
            return {
              ...
              value: {
                ...
                port,
                tray: trayEnabled,
                trayNotify: cfg.trayNotify !== false,
                traySurvivesDsh: cfg.traySurvivesDsh !== false,
                openMode,
                closeToExit: cfg.closeToExit !== false,
                closeToExitDebounceSeconds: debounceSeconds,
                closeToExitFinalConfirmSeconds: finalConfirmSeconds,
                modules: cfg.modules ?? {},
          ...
          case 'config.set': {
            ...
            if (!settingsScope) return { ok: false, error: { code: 'config', message: 'settings service unavailable', details: {} } };
            try {
              const values = (_payload && _payload.values) ?? {};
              const patch = {};
              if (values.launchCommand !== undefined) patch.launchCommand = String(values.launchCommand).trim() || undefined;
              if (values.shortcutName !== undefined) patch.shortcutName = String(values.shortcutName).trim() || undefined;
              if (values.port !== undefined) patch.port = Math.max(1, Math.floor(Number(values.port) || 3080));
              ...
              if (values.closeToExitDebounceSeconds !== undefined) patch.closeToExitDebounceSeconds = Math.max(5, Math.floor(Number(values.closeToExitDebounceSeconds) || 20));
              if (values.closeToExitFinalConfirmSeconds !== undefined) patch.closeToExitFinalConfirmSeconds = Math.max(1, Math.floor(Number(values.closeToExitFinalConfirmSeconds) || 2));
              ...
              logMsg(`[settings] config.set raw values: ${JSON.stringify(_payload?.values ?? {})}`);
              logMsg(`[settings] config.set validated patch: ${JSON.stringify(patch)}`);
              ...
              try {
                const fresh = { ...config, ...settingsScope.get() };
                ...
                logMsg(`[settings] hot-apply: regenerating artifacts with merged config (port=${fPort}, shortcutName=${JSON.stringify(fShortcutName)}, openMode=${fOpenMode}, tray=${fTrayEnabled}, traySurvivesDsh=${fSurvives})`);
                ...
                createDesktopShortcut(fShortcutName, vbsPath, ensureIcon(launcherDir), true);
                logMsg('[settings] hot-apply: artifacts regenerated ok');
                const trayRelatedKeys = ['tray', 'traySurvivesDsh', 'port', 'openMode', 'shortcutName', 'launchCommand'].filter(k => k in patch);
                if (trayRelatedKeys.length > 0) {
                  logMsg(`[settings] hot-apply: tray-related fields changed [${trayRelatedKeys.join(', ')}] -> restarting tray (survives=${fSurvives})`);
                  ...
                  else logMsg('[settings] hot-apply: tray disabled by config, not respawning');
                  restartHint = '已保存并即时应用（托盘已按新配置重启）— 关闭语义等运行参数仍建议重启一次';
                } else {
                  logMsg('[settings] hot-apply: no tray-related change, skip tray restart');
                }
              } catch (error) {
                logMsg(`[settings] hot-apply FAILED after save (config persisted, restart dsh to recover): ${error?.stack ?? error}`);
          ...
          case 'launcher.uninstall': {
            ...
            try {
              ...
              steps.push(killed.length ? `已停止托盘进程: PID ${killed.join(', ')}` : '没有运行中的托盘进程');
              logMsg(`[uninstall] tray killed: ${killed.join(',') || '(none)'}`);
            } catch (error) {
              steps.push(`停止托盘失败（继续）: ${error}`);
            }
            try {
              // 2) 删除桌面快捷方式
              const desktop = resolveDesktopPath() ?? '';
              const lnk = desktop ? join(desktop, `${shortcutName}.lnk`) : '';
              if (lnk && existsSync(lnk)) { unlinkSync(lnk); steps.push(`已删除桌面快捷方式: ${lnk}`); }
              else steps.push('桌面快捷方式不存在（跳过）');
            } catch (error) {
              steps.push(`删除快捷方式失败（继续）: ${error}`);
            }
            try {
              ...
              for (const name of artifacts) {
                ...
                if (existsSync(p)) { try { unlinkSync(p); removed++; } catch { /* 个别被占用跳过 */ } }
              }
              steps.push(`已清理生成物 ${removed} 个文件`);
            } catch (error) {
              steps.push(`清理生成物失败（继续）: ${error}`);
            }
            try {
              ...
              steps.push(reg.status === 0 ? '已删除通知标识注册表项 (AUMID DshNativeLauncher)' : '通知注册表项不存在（跳过）');
            } catch (error) {
              steps.push(`清理注册表失败（继续）: ${error}`);
            }
            try {
              ...
              if (existsSync(profilesRoot)) {
                for (const entry of readdirSync(profilesRoot, { withFileTypes: true })) {
                  ...
                  touched = true;
                  steps.push(`已从 profile "${entry.name}" 移除插件条目（备份: package.json.before-uninstall）`);
                  if (String(spec).startsWith('link:')) {
                    steps.push('检测到开发链接安装（link:），插件源码目录已保留未删除');
                  }
            ...
            manual.push('重启 dsh 后卸载完全生效（启动器不会再生成任何内容）；如启动报依赖错误，在对应 profile 目录执行一次包管理器安装命令即可');
      ...
      if (existsSync(trayPath)) {
        ...
        if (trayRunning && runningVersion !== TRAY_SCRIPT_VERSION) {
          ...
          try {
            ...
            afterKill.on('close', () => {
              logMsg(`[native-launcher] after-kill probe: ${afterOut.trim() || '(empty)'} tray process(es) remain`);
            });
        } else if (trayRunning) {
  ...
  try {
    ...
    logMsg(`[native-launcher] module loading failed: ${error?.stack ?? error}`);
  }
