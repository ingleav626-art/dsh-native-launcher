              ...
              if (existsSync(profilesRoot2)) {
                for (const entry of readdirSync(profilesRoot2, { withFileTypes: true })) {
                  ...
                  try {
                    ...
                    const r = spawnSync('pnpm', ['install', '--lockfile-only', '--ignore-scripts'], { cwd: dir, encoding: 'utf8', windowsHide: true, timeout: 60000, shell: true });
                    logMsg(`[uninstall] lockfile sync in profile "${entry.name}": exit=${r.status}`);
                    if (r.status !== 0) manual.push(`profile "${entry.name}" 的 lockfile 未能自动更新——重启若报依赖错误，在该目录执行一次 pnpm install`);
                  } catch (e) {
