...
import { LAUNCHER_FIELDS, registerLauncherSettings } from "./host/settings.js";
import { getSettingsService } from "./host/settingsScope.js";
import { NOTIFICATION_SETTINGS_SCHEMA } from "./modules/notification/index.js";
...
import { readTrayState, writeWebuiUrl, readShortcutRegistry, migrateLegacyStateFiles } from "./host/state.js";
...
var inject = ["webServer", "connection", "sessionProjections"];
var Config = z.object({
  ...LAUNCHER_FIELDS,
...
function armExitCleanup(dir) {
  ...
  process.once("exit", () => {
    ...
    if (!target) return;
    try {
      for (const name2 of ["launch.cmd", "launcher.vbs", "tray.ps1", "open-webui.ps1", "dsh-webui.ico", "tray-pid.txt", "tray-version.txt", "tray-state.json", "notify-sound.wav", "notify-sound.mp3", "notify-sound.wma"]) {
...
async function applyInner(ctx, config = {}) {
  ...
  try {
    ...
    logMsg(`environment: plugin=${"1.0.0"} node=${process.version} os=${os.type()} ${os.release()} (${os.arch()})`);
