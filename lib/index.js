...
function armExitCleanup(dir) {
  ...
  process.once("exit", () => {
    ...
    if (!target) return;
    try {
      for (const name2 of ["launch.cmd", "launcher.vbs", "tray.ps1", "open-webui.ps1", "dsh-webui.ico", "tray-pid.txt", "tray-version.txt"]) {
...
function applyInner(ctx, config = {}) {
  ...
  try {
    ...
    logMsg(`environment: plugin=${"0.4.0"} node=${process.version} os=${os.type()} ${os.release()} (${os.arch()})`);
