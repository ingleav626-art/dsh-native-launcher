...
import { existsSync, mkdirSync, writeFileSync, copyFileSync, readFileSync, unlinkSync, appendFileSync, readdirSync, statSync, renameSync } from 'node:fs';
...
function killExistingTrays(launcherDir, logMsg) {
  ...
  logMsg(`tray kill result: ${out || '(none)'} exit=${result.status}`);
  return out;
}
...
function startTrayProcess(launcherDir, trayPath, preferPersistent, logMsg) {
  ...
  const spawnTray = (attempt) => {
    ...
    setTimeout(() => {
      probeAlive((alive) => {
        logMsg(`tray spawn attempt ${attempt}: alive=${alive}${alive ? '' : ' -> retrying'}`);
