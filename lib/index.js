...
/** 生成托盘脚本（PowerShell + WinForms NotifyIcon，系统自带零依赖；单实例互斥 + 停止二次确认 + 任务通知气泡）。 */
function writeTrayScript(launcherDir, port, iconPath, openScriptPath) {
  const url = `http://127.0.0.1:${String(port)}`;
  const ps = [
    "$ErrorActionPreference = 'SilentlyContinue'",
    ...
    "# 打开 WebUI：走 open-webui.ps1（与快捷方式一致：已安装 PWA 应用优先，未安装回退浏览器）",
    "$openItem = $menu.Items.Add('打开 WebUI')",
    '$openItem.Add_Click({ if (Test-Path $openScript) { Start-Process powershell -ArgumentList @(\'-NoProfile\', \'-WindowStyle\', \'Hidden\', \'-ExecutionPolicy\', \'Bypass\', \'-File\', $openScript) -WindowStyle Hidden } else { Start-Process $url } })',
    ...
    "# 退出托盘：承载应用生死——DSH 在运行则询问是否一并停止（与原生应用的托盘语义一致）",
    "$exitItem = $menu.Items.Add('退出托盘')",
    `$exitItem.Add_Click({ $line = netstat -ano | Select-String 'LISTENING' | Select-String '0.0.0.0:${String(port)}'; $dpid = if ($line) { ($line -split '\\s+')[-1] } else { '' }; $ask = if ($line) { [System.Windows.Forms.MessageBox]::Show(\"DSH 服务正在运行。退出托盘将同时停止它？(PID $dpid)\", 'DSH WebUI', [System.Windows.Forms.MessageBoxButtons]::YesNo) } else { [System.Windows.Forms.MessageBox]::Show('确定退出托盘？', 'DSH WebUI', [System.Windows.Forms.MessageBoxButtons]::YesNo) }; if ($ask -eq [System.Windows.Forms.DialogResult]::Yes) { if ($line) { taskkill /PID $dpid /T /F | Out-Null }; $notify.Visible = $false; $mutex.ReleaseMutex(); [System.Windows.Forms.Application]::Exit(); exit 0 } })`,
    ...
    "      $msg = switch ($j.kind) {",
    "        'error' { \"会话 $($j.sessionId) 出错\" }",
    "        'aborted' { \"会话 $($j.sessionId) 已中止\" }",
    "        'blocked' { \"会话 $($j.sessionId) 等待你处理（批准或回答）\" }",
    "        'max-tokens' { \"会话 $($j.sessionId) 达到 Token 上限\" }",
    "        default { \"会话 $($j.sessionId) 已完成\" }",
    '      }',
...
/**
 * 任务检测 + 通知（集成升级自 dsh-notification 的设计，MIT 致谢见 README）：
 * 监听 session/event 的 turn/end（权威结束原因：completed/error/aborted/blocked/max-tokens），
 * 写 notify.json 供托盘气泡（页面外）+ client 轮询决策（页面内，per-outcome 开关）。
 * 任何失败只记日志，绝不拖垮宿主。
 */
function setupTaskWatch(ctx, launcherDir, enabled) {
  ...
  try {
    ...
    // 启动静默期：dsh 启动会重放历史 session/event（turn/end 重放 = 轰炸），
    // 60 秒内不写入 notify.json；配合 client 的 outOfViewOnly（页面可见不通知）双保险
    const silentUntil = Date.now() + 60_000;
    // 节流：同一会话 30 秒内只记一次（防连发）
    ...
    const onSessionEvent = (session, event) => {
      try {
        ...
        const entry = { at: now, kind, sessionId: sessionId.slice(0, 8), turn: event.data?.turn ?? 0 };
        writeFileSync(notifyFile, JSON.stringify(entry), 'utf-8');
        console.error(`[native-launcher] task ${kind}: ${sessionId}`);
      } catch (error) {
        console.error(`[native-launcher] task watch handler error: ${error}`);
      }
    };
    ctx.on('session/event', onSessionEvent);
    console.error('[native-launcher] task watch active (turn/end)');
  } catch (error) {
...
function applyInner(ctx, config = {}) {
  ...
  try {
    ...
    if (trayEnabled) writeTrayScript(launcherDir, port, join(launcherDir, 'dsh-webui.ico'), join(launcherDir, 'open-webui.ps1'));
  } catch (error) {
