...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  exit 0
    ...
    '    }',
    '    $rm = Remove-Item $trayNotifyFile -Force -ErrorAction SilentlyContinue -PassThru',
    '    if (-not $rm) { try { Add-Content -Path $trayNotifyLog -Value ((\'[\' + (Get-Date -Format \'HH:mm:ss.fff\') + \'] [toast] WARNING: json remove failed\')) -Encoding UTF8 } catch { } }',
    '    Remove-Item $trayNotifyFile -Force -ErrorAction SilentlyContinue',
    '  } catch {',
...
function applyInner(ctx, config = {}) {
  ...
  try {
    ...
    let notifyProjectionOn = false; // 通知链路打点用：notifications 模块是否成功注册投影
  ...
  try {
    ...
    ctx.on('session/event', (session, event) => {
      ...
      // 链路打点：每个到达的 session 事件都留痕（含被忽略的）——无日志路径即排查盲区
      logMsg(`[notify-chain] session/event: type=${type || '(none)'} turn=${data && data.turn !== void 0 ? data.turn : '?'} session=${String(session && session.id ? session.id : '?')}`);
      if (!type || !data) return;
