...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const ps = [
    ...
    `$exitItem.Add_Click({ $ask = [System.Windows.Forms.MessageBox]::Show('确定退出 DSH WebUI？将停止服务并关闭应用窗口。', 'DSH WebUI', [System.Windows.Forms.MessageBoxButtons]::YesNo); if ($ask -ne [System.Windows.Forms.DialogResult]::Yes) { return }; $appId = '${(appId || '').replace(/'/g, "''")}'; $killTargets = @(); foreach ($p in (Get-CimInstance Win32_Process -Filter "Name='msedge.exe' or Name='chrome.exe'" -ErrorAction SilentlyContinue)) { $cl = $p.CommandLine; if (-not $cl) { continue }; if ($appId -and $cl.Contains("--app-id=$appId")) { $killTargets += $p.ProcessId; continue }; if ($cl.Contains('--app=${url}')) { $killTargets += $p.ProcessId; continue }; if ((-not $appId) -and $cl.Contains('--app-id=')) { $killTargets += $p.ProcessId } }; foreach ($procId in ($killTargets | Select-Object -Unique)) { taskkill /PID $procId /T /F | Out-Null }; $line = netstat -ano | Select-String 'LISTENING' | Select-String '0.0.0.0:${String(port)}'; if ($line) { $dpid = ($line -split '\\s+')[-1]; taskkill /PID $dpid /T /F | Out-Null }; $notify.Visible = $false; $mutex.ReleaseMutex(); [System.Windows.Forms.Application]::Exit(); exit 0 })`,
