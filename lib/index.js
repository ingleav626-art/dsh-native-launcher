...
function writeOpenScript(launcherDir, port, openMode, shortcutName, pwaAppId) {
  ...
  const ps = [
    ...
    '$browserBefore = @(Get-Process msedge,chrome,brave,firefox -ErrorAction SilentlyContinue).Count',
    'function Test-WebUiUp {',
    '  if ($browserBefore -gt 0) { return $true }',
    '  Start-Sleep -Seconds 5',
    '  return ((@(Get-Process msedge,chrome,brave,firefox -ErrorAction SilentlyContinue).Count) -gt 0)',
    '}',
    '',
    ...
    ...(pwaAppId
      ? [
          ...
          "    $runningApps = @(Get-CimInstance Win32_Process -Filter \"Name='msedge.exe' or Name='chrome.exe'\" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('--app-id=' + $appId) })",
          ...
          "      $shellA = New-Object -ComObject Shell.Application",
          "      $afA = $shellA.Namespace('shell:::{4234d49b-0245-4df3-b780-3893943456e1}')",
          '      $activated = $false',
          '      if ($afA) {',
          "        $hostFpA = '${hostForFingerprint.replace(/[^0-9a-zA-Z.:-]/g, '')}-'",
          '        foreach ($appItemA in $afA.Items()) {',
          "          if ($appItemA.Path -like \"$hostFpA*\") { Start-Process 'explorer.exe' -ArgumentList ('shell:AppsFolder\\' + $appItemA.Path); $activated = $true; break }",
          '        }',
          '      }',
          "      if (-not $activated) { Start-Process $m.Groups[1].Value -ArgumentList @(\"--app-id=$appId\") }",
          "      Log-Open ('activated via AppsFolder: ' + $activated)",
          '      exit 0',
          '    }',
          "    Log-Open ('app not detected running; dump browser proc cmdlines (window process should be here):')",
          '    foreach ($pd in @(Get-CimInstance Win32_Process -Filter "Name=\'msedge.exe\' or Name=\'chrome.exe\'" -ErrorAction SilentlyContinue)) {',
          '      $pcl = $pd.CommandLine',
          '      if (-not $pcl) { continue }',
          ...
          "    $afterApps = @(Get-CimInstance Win32_Process -Filter \"Name='msedge.exe' or Name='chrome.exe'\" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('--app-id=' + $appId) })",
...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const ps = [
    ...
    "# 打开 WebUI：优先直接启动已装 PWA 应用（AppsFolder 枚举，跳过 PowerShell 中间层，更快）；",
    "# 未安装时回退 open-webui.ps1 / 默认浏览器",
    "$openItem = $menu.Items.Add('打开 WebUI')",
    '$openItem.Add_Click({',
    '  $launched = $false',
    "  $bBefore = @(Get-Process msedge,chrome,brave,firefox -ErrorAction SilentlyContinue).Count",
    '  try {',
    ...
    "        if ($app.Path -like '127.0.0.1-*') { Start-Process 'explorer.exe' -ArgumentList ('shell:AppsFolder\\' + $app.Path); $launched = $true; break }",
    '      }',
    '    }',
    '  } catch { }',
    '  if ($launched) {',
    ...
    '    Start-Sleep -Seconds 5',
    '    if ((@(Get-Process msedge,chrome,brave,firefox -ErrorAction SilentlyContinue).Count) -gt 0) { return }',
    '    Start-Process $url',
    '    return',
    '  }',
    '  if (-not $launched) {',
  ...
  foreach ($p in $procs) {
    ...
    if ($cl.Contains('--app=${url}')) { $killTargets += $p.ProcessId; Log-Exit '    -> match --app'; continue }
  ...
  $after = @(Get-CimInstance Win32_Process -Filter "Name='msedge.exe' or Name='chrome.exe'" -ErrorAction SilentlyContinue)
