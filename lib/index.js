  ...
  const ps = [
    ...
    ...(pwaAppId
      ? [
          ...
          "    $runningApps = @(Get-CimInstance Win32_Process -Filter \"Name='msedge.exe' or Name='chrome.exe' or Name='pwahelper.exe'\" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('--app-id=' + $appId) })",
          ...
          `        $primaryAppNameA = '${lnkBase.replace(/'/g, "''")}'`,
          '        foreach ($appItemA in $afA.Items()) {',
          "          # AUMID 匹配放宽：站点指纹 127.0.0.1- / localhost-（Edge 可能用任一 host 形式），或显示名匹配",
          "          if ($appItemA.Path -like \"$hostFpA*\" -or $appItemA.Path -like 'localhost-*' -or $appItemA.Name -eq $primaryAppNameA) {",
          "            Start-Process 'explorer.exe' -ArgumentList ('shell:AppsFolder\\' + $appItemA.Path)",
          '            $activated = $true',
          "            Log-Open ('AppsFolder matched: Path=' + $appItemA.Path + ' Name=' + $appItemA.Name)",
          '            break',
          '          }',
          '        }',
          '        if (-not $activated) {',
          '          foreach ($appItemA in $afA.Items()) {',
          "            Log-Open ('AppsFolder item: Path=' + $appItemA.Path + ' Name=' + $appItemA.Name)",
          '          }',
          '        }',
          '      }',
          ...
          "    $afterApps = @(Get-CimInstance Win32_Process -Filter \"Name='msedge.exe' or Name='chrome.exe' or Name='pwahelper.exe'\" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('--app-id=' + $appId) })",
