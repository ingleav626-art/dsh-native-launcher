...
function writeOpenScript(launcherDir, port, openMode, shortcutName, pwaAppId) {
  ...
  const ps = [
    ...
    '    if ($script:focused -ne [IntPtr]::Zero) {',
    ...
    ...(pwaAppId
      ? [
          ...
          '      $focusOk = Focus-AppWindow ([int]$runningApps[0].ProcessId)',
...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const ps = [
    ...
    '  if ($appId) {',
    "    $appProcs = @(Get-CimInstance Win32_Process -Filter \"Name='msedge.exe' or Name='chrome.exe' or Name='pwahelper.exe'\" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('--app-id=' + $appId) })",
    '    if ($appProcs.Count -gt 0) {',
    '      if (Focus-AppWindow ([int]$appProcs[0].ProcessId)) { return }',
    '    }',
    '  }',
    '  $launched = $false',
