      ...
      if (existsSync(trayPath)) {
        ...
        if (!trayRunning || runningVersion !== TRAY_SCRIPT_VERSION) {
          ...
          const spawnTray = (attempt) => {
            ...
            // 注意：不能用 detached: true——在部分 Windows 配置下
            // detached + PowerShell 会让进程立即退出（exit 0，脚本一行不执行）。
            // child.unref() 已确保 dsh 退出后托盘进程继续存活，不需要 detached。
            const child = spawn(
              'powershell',
            ...
            child.on('exit', (code, signal) => {
              logMsg(`[native-launcher] tray attempt ${attempt} exited code=${code} signal=${String(signal)} (stdio=ignore; see tray-exit.log for script-level reason)`);
            });
            // 等待 1.5s 后验证存活（脚本启动 + mutex 获取需要时间），失败则异步重试
            setTimeout(() => {
              probeAlive((alive) => {
