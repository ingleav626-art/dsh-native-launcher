      ...
      async (endpoint, _payload) => {
        switch (endpoint) {
          ...
          case 'launcher.uninstall': {
            ...
            logU('INFO', `════ uninstall session start (dsh pid=${process.pid}, launcherDir=${launcherDir}, clearSettings=${clearSettings}) ════`);
            ...
            try {
              ...
              //    三态上报：REMOVED=成功 / FAILED=删除失败(含原因)——失败明细全部进审计日志
              const desktop = resolveDesktopPath() ?? '';
              let removedLnk = 0;
              const failedLnks = [];
              if (desktop) {
                const wsPs = [
                  ...
                  `  try { $s = $ws.CreateShortcut($_.FullName); if ($s.TargetPath -match 'wscript' -and $s.Arguments -match 'dsh-webui-launcher.*launcher\\.vbs') {`,
                  `    try { Remove-Item $_.FullName -Force -ErrorAction Stop; Write-Output ('REMOVED:' + $_.Name) }`,
                  `    catch { Write-Output ('FAILED:' + $_.Name + ' :: ' + $_.Exception.Message) } } catch { }`,
                  '} | Sort-Object -Unique',
                ].join('; ');
                const r = spawnSync('powershell', ['-NoProfile', '-NonInteractive', '-Command', wsPs], { encoding: 'utf8', windowsHide: true, timeout: 15000 });
                for (const line of String(r.stdout ?? '').split(/\r?\n/).map(s => s.trim()).filter(Boolean)) {
                  if (line.startsWith('REMOVED:')) { removedLnk++; logU('INFO', `STEP 2/5 removed: ${line.slice(8)}`); }
                  else if (line.startsWith('FAILED:')) {
                    failedLnks.push(line.slice(7));
                    const busy = /being used by another process|拒绝访问|正在使用/i.test(line);
                    logU('ERROR', `STEP 2/5 shortcut delete FAILED: ${line}${busy ? ' (hint: file locked - retry after restarting explorer or closing apps)' : ''}`);
                  }
                }
                if (r.status !== 0 || r.stderr) logWarn(`[uninstall] STEP 2/5 sweep powershell exit=${r.status} stderr=${String(r.stderr ?? '').trim().slice(0, 200)}`);
              }
              addStep(removedLnk > 0 ? `已删除 ${removedLnk} 个桌面快捷方式${failedLnks.length ? `（${failedLnks.length} 个失败，详见 uninstall.log）` : ''}` : '桌面无本插件快捷方式（跳过）');
              logU('INFO', `STEP 2/5 shortcut-sweep summary: removed=${removedLnk}, failed=${failedLnks.length}`);
            } catch (error) {
            ...
            manual.push('重启 dsh 后卸载完全生效（启动器不会再生成任何内容）；诊断日志保留在 .dsh-webui-launcher\\ 供排查，确认无误后可手动删除整个目录');
