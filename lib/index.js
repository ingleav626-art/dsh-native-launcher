...
function applyInner(ctx, config = {}) {
  ...
  // 3.6 通知诊断日志接收：client 端通知决策（runner/测试按钮）经此上报，
  //     写入 native-launcher.log（不污染浏览器 console；排查"通知不显示"）
  try {
    harness.handle('ntf-log', async (args) => {
      logMsg(`[ntf] ${JSON.stringify(args ?? {})}`);
      return { ok: true };
    });
  } catch (error) {
    logMsg(`[native-launcher] ntf-log handle failed: ${error}`);
  }
