...
export const name = 'better-launcher';
...
function writeTrayScript(launcherDir, port, iconPath) {
  ...
  const ps = [
    ...
    "$mutex = New-Object System.Threading.Mutex($false, 'Local\\BetterDshWebuiLauncherTray')",
...
function ensureIcon(launcherDir) {
  try {
    ...
    if (!existsSync(target) || readFileSync(target).length !== readFileSync(ICON_RESOURCE).length) {
      ...
      console.error('[webui-launcher] icon copied/updated');
    }
    return target;
  } catch (error) {
    console.error(`[webui-launcher] icon copy failed: ${error}`);
    return null;
  }
}
...
function createDesktopShortcut(shortcutName, vbsPath, iconPath, force) {
  ...
  if (!force && existsSync(lnk)) {
    ...
    try {
      ...
      if (buf.includes(vbsNeedle) && (!iconNeedle || buf.includes(iconNeedle))) {
        console.error(`[webui-launcher] shortcut already exists and points to current vbs + icon, skipping: ${lnk}`);
        return;
      }
      console.error(`[webui-launcher] shortcut exists but points elsewhere or icon changed, recreating: ${lnk}`);
    } catch {
      // 读失败（权限/损坏）→ 保守重建
      console.error(`[webui-launcher] shortcut unreadable, recreating: ${lnk}`);
    }
  }
  const ps = [
    `$ws = New-Object -ComObject WScript.Shell`,
  ...
  try {
    ...
    console.error(`[webui-launcher] shortcut created: ${lnk}`);
  } catch (error) {
    console.error(`[webui-launcher] shortcut creation failed: ${error}`);
  }
}
...
function ensureSquareIconPng(launcherDir, pngBuffer) {
  try {
    ...
    if (result.status === 0 && existsSync(outPath)) {
      console.error('[better-launcher] square icon generated (256x256)');
      return readFileSync(outPath);
    }
    console.error('[better-launcher] square icon generation failed, falling back to source png');
  } catch (error) {
    console.error(`[better-launcher] square icon failed: ${error}`);
  }
  return pngBuffer;
}
...
function registerPwaRoutes(ctx, launcherDir, iconPath) {
  ...
  if (!png || png.length === 0) {
    console.error('[better-launcher] pwa icon extract failed, routes skipped');
    return;
  }
  const manifestJson = JSON.stringify({
    ...
    theme_color: '#141414',
    icons: [{ src: '/better-launcher/icon.png', sizes: '256x256', type: 'image/png', purpose: 'any' }],
  });
  try {
    ctx.webServer.register({
      kind: 'exact',
      path: '/better-launcher/manifest.webmanifest',
      handler: (_req, res) => {
    ...
    ctx.webServer.register({
      kind: 'exact',
      path: '/better-launcher/icon.png',
      handler: (_req, res) => {
        res.writeHead(200, { 'Content-Type': 'image/png', 'Cache-Control': 'no-store' });
        res.end(png);
      },
    });
    console.error('[better-launcher] pwa routes registered');
  } catch (error) {
    console.error(`[better-launcher] pwa route registration failed: ${error}`);
  }
}
...
function openBrowser(port, launcherDir) {
  try {
    ...
    console.error(`[better-launcher] open browser failed: ${error}`);
    return false;
  }
}
...
export function apply(ctx, config = {}) {
  try {
    ...
    console.error(`[better-launcher] apply failed (harness continues): ${error?.stack ?? error}`);
  }
}
...
function applyInner(ctx, config = {}) {
  ...
  const home = process.env.USERPROFILE;
  if (!home) {
    console.error('[webui-launcher] USERPROFILE missing; launcher cannot be installed');
    return;
  }
  ...
  try {
    ...
    console.error(`[webui-launcher] launcher script failed: ${error}`);
  }
  ...
  try {
    ...
    console.error(`[better-launcher] rpc registration failed: ${error}`);
  }
  ...
  if (trayEnabled) {
    try {
      ...
      console.error(`[better-launcher] tray spawn failed: ${error}`);
    }
  }
