...
let APPLY_SEQ = 0;
let SAVE_SEQ = 0;
// 一键卸载后置标记：dsh 进程退出瞬间清空整个 launcherDir（uninstall 设置，exit 钩子消费）
let pendingDirWipe = null;
      ...
      async (endpoint, _payload) => {
        switch (endpoint) {
          ...
          case 'launcher.uninstall': {
            ...
            try {
              // 2) 删除桌面快捷方式——收敛式：扫所有指向本插件 launcher.vbs 的 lnk 全删，
              //    不依赖配置名（历史改名/快照过期都不会残留）
              const desktop = resolveDesktopPath() ?? '';
              let removedLnk = 0;
              if (desktop) {
                const wsPs = [
                  `$ws = New-Object -ComObject WScript.Shell`,
                  `Get-ChildItem '${desktop.replace(/'/g, "''")}' -Filter *.lnk -ErrorAction SilentlyContinue | ForEach-Object {`,
                  `  try { $s = $ws.CreateShortcut($_.FullName); if ($s.TargetPath -match 'wscript' -and $s.Arguments -match 'dsh-webui-launcher.*launcher\\.vbs') { Remove-Item $_.FullName -Force; $_.Name } } catch { }`,
                  '} | Sort-Object -Unique',
                ].join('; ');
                const r = spawnSync('powershell', ['-NoProfile', '-NonInteractive', '-Command', wsPs], { encoding: 'utf8', windowsHide: true, timeout: 15000 });
                removedLnk = String(r.stdout ?? '').split(/\r?\n/).filter(Boolean).length;
              }
              steps.push(removedLnk > 0 ? `已删除 ${removedLnk} 个桌面快捷方式` : '桌面无本插件快捷方式（跳过）');
            } catch (error) {
            ...
            // lockfile 同步：package.json 已改，同步 pnpm-lock.yaml，避免下次启动报依赖不一致
            try {
              ...
              if (existsSync(profilesRoot2)) {
                for (const entry of readdirSync(profilesRoot2, { withFileTypes: true })) {
                  ...
                  try {
                    ...
                    // 本机 pnpm 经 corepack 管理（不在 PATH），用 shell 解析 corepack/pnpm 双通道
                    const r = spawnSync('corepack', ['pnpm', 'install', '--lockfile-only', '--ignore-scripts'], { cwd: dir, encoding: 'utf8', windowsHide: true, timeout: 60000, shell: true });
                    let status = r.status;
                    if (status !== 0) {
                      // corepack 不存在时退回直呼 pnpm
                      const r2 = spawnSync('pnpm', ['install', '--lockfile-only', '--ignore-scripts'], { cwd: dir, encoding: 'utf8', windowsHide: true, timeout: 60000, shell: true });
                      status = r2.status;
                    }
                    logMsg(`[uninstall] lockfile sync in profile "${entry.name}": exit=${status}`);
                    if (status !== 0) manual.push(`profile "${entry.name}" 的 lockfile 未能自动更新——重启若报依赖错误，在该目录执行一次 pnpm install`);
                  } catch (e) {
                    manual.push(`profile "${entry.name}" lockfile 同步失败（重启若报依赖错误，在该目录执行 pnpm install）: ${e?.message ?? e}`);
                  }
                }
              }
            } catch { /* lockfile 同步失败不阻塞卸载 */ }
            manual.push('如安装过 PWA 应用：浏览器 edge://apps 中手动卸载"DSH WebUI"');
            // 终极清理：dsh 进程退出瞬间删掉整个 launcherDir（届时日志/pid 等运行期文件已无主）
            pendingDirWipe = launcherDir;
            process.once('exit', () => {
              const target = pendingDirWipe; pendingDirWipe = null;
              if (!target) return;
              try { rmSync(target, { recursive: true, force: true }); } catch { }
            });
            manual.push('重启 dsh 后卸载完全生效（启动器不会再生成任何内容）');
