...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  const ps = [
    ...
    `try { Add-Content -Path '${join(launcherDir, 'tray-version.txt').replace(/'/g, "''")}' -Value ($trayVersion.ToString()) -NoNewline -Encoding UTF8 -Force } catch { }`,
    '',
  ...
  exit 0
    ...
    '    $ok = Show-TrayToast $t $b',
    '    if (-not $ok) {',
    '      # Toast 失败兜底：BalloonTip + 系统提示音',
  ...
  if (trayEnabled) {
    try {
      ...
      if (existsSync(trayPath)) {
        ...
        if (runningVersion !== TRAY_SCRIPT_VERSION) {
          ...
          try {
            spawnSync(
              ...
              [
                ...
                `Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('tray.ps1') -and $_.CommandLine.Contains('${launcherDir.replace(/'/g, "''")}') } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }`,
              ],
          ...
          try {
            spawnSync('powershell', ['-NoProfile', '-NonInteractive', '-Command', 'Start-Sleep -Milliseconds 600'], {
              stdio: 'ignore',
              windowsHide: true,
            });
          } catch {
    ...
    ctx.on('session/event', (session, event) => {
      ...
      if (type === 'turn/start') {
        ...
        const timer = setTimeout(() => {
          ...
          logMsg(`[tray-notify] host fallback (no client report): ${key}`);
        }, 2000);
