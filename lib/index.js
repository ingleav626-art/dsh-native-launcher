...
function applyInner(ctx, config = {}) {
  ...
  try {
    if (ctx.settings) {
      ...
      cfg = { ...config, ...scope.get() };
      scope.watch(() => logMsg('[native-launcher] settings updated — restart dsh (double-click shortcut) to fully apply'));
    } else {
      ...
      async (endpoint, _payload) => {
        switch (endpoint) {
          ...
          case 'launcher.uninstall': {
            ...
            // 插件自身无法删除自己（运行中），profile 移除与 PWA 卸载返回 manual 步骤由用户完成。
            const steps = [];
            const manual = [
              '从 dsh profile 移除本插件：编辑 E:\\dsh\\profiles\\web\\package.json 删除 dsh-native-launcher 条目后重启（或 dsh plugin remove）',
              '如安装过 PWA 应用：浏览器 edge://apps 中手动卸载"DSH WebUI"',
              '最后可删除残留目录 ' + launcherDir + '（若仍存在）',
            ];
            try {
              // 1) 停托盘进程（含其子进程）
  ...
  try {
    ...
    for (const mod of BUILTIN_MODULES) {
      ...
      const core = { apiVersion: CORE_API_VERSION, config, launcherDir, port, url, log: logMsg };
  ...
  try {
    ...
    const scheduleExitCheck = (reason) => {
      ...
      logMsg('[close-to-exit] no tasks, 20s debounce started');
      exitTimer = setTimeout(() => {
        exitTimer = null;
        logMsg(`[close-to-exit] 20s elapsed: clients=${clients.size}, running=${runningCount}`);
        ...
        logMsg('[close-to-exit] idle, starting 2s final confirm');
