          ...
          case 'config.set': {
            ...
            try {
              ...
              try {
                ...
                logMsg('[native-launcher] artifacts regenerated after config.set');
                const trayRelated = ['tray', 'traySurvivesDsh', 'port', 'openMode', 'shortcutName', 'launchCommand'].some(k => k in patch);
                if (trayRelated) {
                  killExistingTrays(launcherDir, logMsg);
                  if (fTrayEnabled) startTrayProcess(launcherDir, join(launcherDir, 'tray.ps1'), fresh.traySurvivesDsh !== false, logMsg);
                  restartHint = '已保存并即时应用（托盘已按新配置重启）— 关闭语义等运行参数仍建议重启一次';
                }
              } catch (error) {
                logMsg(`[native-launcher] hot-apply after config.set failed (saved anyway): ${error?.stack ?? error}`);
              }
