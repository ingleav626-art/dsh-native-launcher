        ...
        switch (endpoint) {
          ...
          case 'config.set': {
            ...
            try {
              ...
              logMsg(`[native-launcher] config.set: ${JSON.stringify(patch)}`);
              return { ok: true, value: { message: '已保存 — 重启 dsh（双击桌面快捷方式）后完全生效' } };
            } catch (error) {
  ...
  if (trayEnabled) {
    try {
      ...
      if (existsSync(trayPath)) {
        ...
        // 重试 + 存活验证（全部异步：spawn 后 1.5s 异步 probe，不阻塞 apply——
        // 同步 spawnSync 会拖住 dsh 初始化，是本插件启动回归的根源）。
        // stdio: 'ignore'：detached + stdio pipe 在 Windows 上会让 PowerShell 启动即退
        // （exit 0、脚本一行不执行、无 tray-exit.log 记录）——与 launch.cmd 的 `start`
        // 方式对照铁证（launch.cmd spawn 的托盘正常写 mutex 日志）。
        if (!trayRunning || runningVersion !== TRAY_SCRIPT_VERSION) {
          const probeAlive = (cb) => {
            ...
            probe.stdout?.on('data', (chunk) => {
              out = String(out + chunk).slice(-2000);
            });
          ...
          const spawnTray = (attempt) => {
            ...
            // 双机制互备（2026-08 跨机实测结论）：
            //  - WScript.Shell.Run(...,0)（launcher.vbs 同款）：独立存活、隐藏窗口、不随父死，
            //    两台机器验证稳定（M3 实测 + 用户双击快捷方式链路）——首选；
            //  - 直 spawn（无 detached）：能拉起，但部分环境（含本机实测）dsh 退出后托盘随父进程死，
            //    仅作兜底。
            // 不用 detached:true——部分 Windows 配置下 detached + PowerShell spawn 即退
            // （exit 0、脚本一行不执行、无 tray-exit.log 记录；PR #2 报告 + 本机/沙箱均复现）。
            // 不用 cmd start——start 创建的控制台窗口黑框无法被 -WindowStyle Hidden 隐藏，
            // 且用户关闭黑框会连带杀掉托盘（PR #2 报告的问题 2 实锤）。
            // attempt 1 = WScript 首选；attempt 2/3 = 直 spawn 兜底（任一 alive 即停）。
            if (attempt === 1) {
              ...
              child.on('exit', (code, signal) => {
                logMsg(`[native-launcher] tray attempt ${attempt} (wscript run) launcher exited code=${code} signal=${String(signal)}`);
              });
            } else {
              const child = spawn(
                'powershell',
              ...
              child.unref();
              child.on('exit', (code, signal) => {
                logMsg(`[native-launcher] tray attempt ${attempt} (direct spawn) exited code=${code} signal=${String(signal)} (stdio=ignore; see tray-exit.log for script-level reason)`);
              });
            }
            // 等待 1.5s 后验证存活（脚本启动 + mutex 获取需要时间），失败则降级/重试
            setTimeout(() => {
              probeAlive((alive) => {
