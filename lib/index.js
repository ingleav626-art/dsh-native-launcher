...
function writeTrayScript(launcherDir, port, iconPath, openScriptPath, appId) {
  ...
  exit 0
    ...
    '      try { Add-Content -Path $trayNotifyLog -Value ((\'[\' + (Get-Date -Format \'HH:mm:ss.fff\') + \'] [toast] consuming: \' + $t) -replace \'\\r\\n\', \' \' ) -Encoding UTF8 } catch { }',
    '      $ok = $false',
...
function applyInner(ctx, config = {}) {
  ...
  // 通知链路打点用：notifications 模块是否成功注册投影（跨 try 块共享，必须函数级声明）
  let notifyProjectionOn = false;
  ...
  try {
    ...
    for (const mod of BUILTIN_MODULES) {
      ...
      const ports = createNotificationPorts(ctx, { launcherDir, log: logMsg, logWarn, logFail });
      const instance = mod.create(ports);
      // start() 返回的 dispose 暂不持有：dsh 退出即回收（将来支持模块热重载时再收集）
      instance.start();
      if (mod.id === 'notifications') { notificationModule = instance; notifyProjectionOn = true; }
  ...
  try {
    ...
    const writeTrayNotify = (title, body, key) => {
      ...
      // 通知模块关闭时一并停用托盘 Toast——modules.notifications=false 的语义是
      // 「本包的全部通知能力让位」（用户可能已选用其他通知插件，避免双弹冲突）
      if ((cfg.modules ?? {}).notifications === false) { logMsg(`[tray-notify] suppressed by config (modules.notifications=false): ${key}`); return; }
      try {
    ...
    ctx.on('session/event', (session, event) => {
      ...
      if (!type || !data) return;
      // 只对链路相关事件打点；assistant/chunk 等高频事件直接跳过（否则日志洪水淹没决策行）
      if (type !== 'turn/start' && type !== 'turn/end') return;
      logMsg(`[notify-chain] session/event: type=${type} turn=${data.turn} session=${String(session && session.id ? session.id : '?')}`);
      const key = `${String(session && session.id ? session.id : '?')}:${data.turn}`;
      if (type === 'turn/start') {
        if (runningCount > 0) {
          liveTurns.add(key);
          logMsg(`[notify-chain] turn/start tracked: ${key} (running=${runningCount})`);
        } else {
          logWarn(`[notify-chain] turn/start IGNORED (runningCount=0, agent/status may lag): ${key}`);
        }
      } else if (type === 'turn/end') {
        if (!liveTurns.has(key)) {
          // 关键静默点显性化：turn/end 到达但 start 未被跟踪 = 通知链路在此中断
          logWarn(`[notify-chain] turn/end DROPPED (start not tracked): key=${key} kind=${(data.reason && data.reason.kind) || 'completed'} running=${runningCount} liveTurns=[${Array.from(liveTurns).join(',')}]`);
          return;
        }
        liveTurns.delete(key);
        const endKind = data.reason && data.reason.kind ? data.reason.kind : 'completed';
        // 通知链路打点：turn/end 到达即留痕（投影是否注册 / 客户端在线数）——
        // 排查"任务完成了为什么没通知"从这里开始对时间线
        logMsg(`[notify-chain] turn/end: key=${key} kind=${endKind} projection=${notifyProjectionOn ? 'on' : 'off'} clients=${clients.size} running=${runningCount}`);
        if (pendingKeys.has(key)) return;
        const kind = endKind;
        const title = reasonTitle[kind] || '任务完成';
        // 2s 确认窗口：client 上报（规则过滤）优先；无上报则 host 兜底
        logMsg(`[notify-chain] 2s window armed: key=${key} (client reports take precedence; host falls back)`);
        const timer = setTimeout(() => {
          pendingKeys.delete(key);
          if (reportedKeys.has(key)) { logMsg(`[notify-chain] fallback skip: client already reported ${key}`); return; }
          // 有客户端在线但未上报 = 被规则/backgroundOnly 抑制（页面开着，决策在 client）→ 尊重设置，不兜底；
          // 仅当无任何客户端在线（页面全关）时才由 host 补发
          if (clients.size > 0) { logMsg(`[notify-chain] fallback skip: clients online (${clients.size}) but no report = suppressed by client rules`); return; }
          let body = '';
          try {
          ...
          logMsg(`[notify-chain] host fallback fired: ${key}`);
        }, 2000);
