...
function startTrayProcess(launcherDir, trayPath, preferPersistent, logMsg) {
  ...
  const spawnTray = (attempt) => {
    if (attempt > 3) {
      logFail('[tray] spawn FAILED after 3 attempts (mutex held or script error; see tray-exit.log)');
      return;
    }
        ...
        switch (endpoint) {
          ...
          case 'config.set': {
            ...
            try {
              ...
              logMsg(`[settings] ${seq} validated patch: ${JSON.stringify(patch)}`);
              ...
              try {
                ...
                if (patch.shortcutName && patch.shortcutName !== shortcutName) {
                  const oldLnk = join(resolveDesktopPath() ?? '', `${shortcutName}.lnk`);
                  if (oldLnk && existsSync(oldLnk)) {
                ...
                if (trayRelatedKeys.length > 0) {
                  logMsg(`[settings] ${seq} tray-related fields changed [${trayRelatedKeys.join(', ')}] -> restarting tray (survives=${fSurvives})`);
                  ...
                  logMsg(`[settings] ${seq} no tray-related change, skip tray restart`);
                }
              } catch (error) {
