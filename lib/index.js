...
// P2 拆分批次（源码在 src/host/**，产物 lib/host/*.js——build.mjs ENTRIES 维护）：
import { logMsg, logWarn, logFail, beginApplyLog, nextSaveSeq } from './host/logger.js';
import { logsDirOf } from './host/paths.js';
...
import { resolveDesktopPath } from './host/paths.js';
import { ensureIcon, extractPngDataUrl } from './host/icon.js';
import { createDesktopShortcut } from './host/shortcut.js';
import { findInstalledPwaAppId, registerPwaRoutes, openBrowser } from './host/pwa.js';
...
// —— RPC 兜底桥（0.1.5-rc.x 实测：官方 connection fiber 的 inject 只剩 ["credentials"]，
...
function registerRpcFallbackBridge(ctx, channel, handler) {
  ...
  const route = {
    ...
    handler: async (req, res) => {
      try {
        ...
        const validEnvelope = !!body && typeof body === 'object'
          && body.type === 'client-request'
          && typeof body.rpcId === 'string' && body.rpcId.length > 0
          && typeof body.method === 'string';
        if (!validEnvelope || body.method !== endpoint) {
          const badId = body && typeof body.rpcId === 'string' ? body.rpcId : '';
          jsonOut(res, 200, {
        ...
        try {
          ...
          jsonOut(res, 200, { type: 'server-response', rpcId: body.rpcId, result });
        } catch (error) {
          res.writeHead(500);
...
function applyInner(ctx, config = {}) {
  ...
  // 共享状态：agent 运行计数（关闭语义任务检查；恒 0 的缺陷重建归 P2·B5）
  let runningCount = 0;
  try {
  ...
  const captureAuthUrl = () => {
    ...
    try {
      ...
      logMsg(`webui token url capture deferred: ${error?.message ?? error}`);
    }
  };
  captureAuthUrl();
  if (!authUrlCaptured) {
  ...
  //    此调用注册即抛错）；失败时自动切 webServer 兜底桥（registerRpcFallbackBridge），
  //    client 端与 wire 协议完全不变。官方修复后主通道自动恢复，兜底不再触发。
  const nativeLauncherRpcHandler = async (endpoint, _payload) => {
        switch (endpoint) {
          ...
          case 'config.set': {
            ...
            try {
              ...
              try {
                ...
                createDesktopShortcut(fShortcutName, vbsPath, ensureIcon(launcherDir, logMsg), false, launcherDir, logMsg);
                ...
                if (trayChanged.length > 0) {
                  ...
                  if (fTrayEnabled) startTrayProcess(launcherDir, join(launcherDir, 'tray.ps1'), fSurvives, logMsg, logWarn, logFail);
          ...
          case 'shortcut.recreate': {
            ...
            createDesktopShortcut(liveName, vbsPath, iconPath, true, launcherDir, logMsg);
          ...
          case 'diagnostics.openLogs': {
            ...
            const logDir = logsDirOf(launcherDir);
            try {
              ...
              logFail(`[diagnostics] 打开日志目录失败：${error?.stack ?? error}`);
          ...
          case 'presence-report': {
            ...
            if (!notificationModule) return { ok: false, error: { code: 'presence', message: 'notification module not loaded', details: {} } };
            const presenceAccepted = notificationModule.reportPresence(_payload ?? {});
          ...
          case 'notification.test': {
            ...
            if (!notificationModule) return { ok: false, error: { code: 'notification', message: 'notification module not loaded', details: {} } };
            return notificationModule.testNotify()
              ? { ok: true }
          ...
          case 'launcher.uninstall': {
            ...
            const ULOG = join(logsDirOf(launcherDir), 'uninstall.log');
          ...
          case 'pending-report': {
            ...
            if (!notificationModule) return { ok: false, error: { code: 'pending', message: 'notification module not loaded', details: {} } };
            const accepted = notificationModule.reportPending(_payload ?? {});
            return accepted
              ? { ok: true }
          ...
          case 'notification.get': {
            ...
            if (!notificationModule) return { ok: false, error: { code: 'notification', message: 'notification module not loaded', details: {} } };
            const notificationSettings = notificationModule.getSettings();
          ...
          case 'notification.set': {
            ...
            const notificationApplied = await notificationModule.updateSettings(notificationPatch);
  ...
  try {
    ctx.connection.rpc.handle('/native-launcher', nativeLauncherRpcHandler, { authority: 'trusted-host' });
    logMsg('[rpc] /native-launcher registered via connection.rpc.handle');
  } catch (error) {
    logMsg(`[rpc] connection.rpc.handle failed: ${error?.message ?? error} — activating webServer bridge fallback`);
    registerRpcFallbackBridge(ctx, '/native-launcher', nativeLauncherRpcHandler);
  }
  ...
  // 4.5 模块加载（容器协议 v2，dev-notes 一·七）：
  //   config.modules[id] 开关（缺省用模块自带 defaultEnabled）；apiVersion 不一致拒载；
  //   端口由 lib/host-ports.js 适配（官方 ctx 只在组装根被触摸，模块只见窄接口）；
  ...
  try {
    ...
    for (const mod of BUILTIN_MODULES) {
      ...
      const compat = typeof mod.apiVersion === 'number' && mod.apiVersion === CORE_API_VERSION;
      ...
      logMsg(`[modules] ${mod.id}: applying (core=v${CORE_API_VERSION})`);
      const ports = createNotificationPorts(ctx, {
      ...
      instance.start();
      if (mod.id === 'notifications') { notificationModule = instance; }
  ...
  // 4.7 关闭语义（托盘通知已收归通知模块：投影 change feed → 投递端，本体不再插手）
  //   页面 online/offline 上报 → 全部离线 + 20s 防抖 + 任务空闲 → appExit(0) 官方优雅退出
  try {
    ...
    /** 等待任务结束期间的复查定时器（见 armIdleProbe；仅等待期间存在）。 */
    let idleProbe = null;
    ...
    const agentsProbe = ctx.get('agents');
    ...
    /**
     * 等待任务结束的复查轮询（**只在实际等待期间存在**）。
     *
     * 为什么必须有它：任务完成时**没有任何事件能通知我们**——旧实现靠 `ctx.on('agent/status')` 的
     * idle 回调，而该订阅因作用域问题 0 次触发（见上）；现在我们只有"按需查询"的同步信号，
     * 于是需要一个轻量复查把"任务跑完了"这刻接上，否则 waitingForIdle 设下后再没人唤醒，
     * dsh 会一直挂着不退出（比"直接退出杀任务"更糟：用户以为关了窗口，服务却在偷跑）。
     * 5s 间隔只存在于等待期间，任务结束/页面回来立刻清掉。
     */
    const armIdleProbe = () => {
      if (idleProbe) return;
  ...
  const waitForPageReady = (port, timeoutMs, cb) => {
    ...
    const probe = () => {
      const req = http.get(probeTarget ?? { host: '127.0.0.1', port, path: '/' }, { timeout: 2000 }, (res) => {
        res.resume();
        if (res.statusCode >= 200 && res.statusCode < 400) { cb(true); return; }
        retry();
      });
      req.on('error', retry);
      req.on('timeout', () => {
